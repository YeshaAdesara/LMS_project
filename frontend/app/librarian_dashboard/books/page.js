  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-indigo-100 selection:text-indigo-900 relative overflow-hidden flex flex-col">
      ...
      <main className="relative z-10 flex-1 max-w-7xl w-full mx-auto px-6 py-10 animate-[fadeIn_0.5s_ease-out]">
        ...
        {message && (
          <div className={`p-4 rounded-xl border flex items-start gap-3 mb-6 animate-[fadeIn_0.3s_ease-out] w-full ${
            message.includes("✅") 
        ...
        {books.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-[24px] border border-dashed border-slate-200 shadow-sm w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            {books.map((book) => {
              ...
              return (
                <div
                  key={book._id}
                  className="bg-white border border-slate-200 rounded-[22px] p-5 shadow-sm flex flex-col justify-between gap-5 transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/50 hover:border-slate-300 animate-[slideUp_0.4s_ease-out] w-full"
                >
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="space-y-1.5 text-xs text-slate-500 font-semibold pt-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>Author: <strong className="text-slate-700 font-bold">{book.author}</strong></span>
                      </div>
                      <div className="flex items-center gap-1.5">
                  ...
                  {/* INTERACTIVE CONTROL COMPONENT PANEL */}
                  <div className="w-full shrink-0 space-y-2 border-t border-slate-100 pt-4">
                    <label className="block text-xs font-bold text-slate-700">Status State</label>
                    <div className="relative w-full">
                      ...
                      <select
                        ...
                        className="w-full pl-9 pr-8 py-3 rounded-xl border border-slate-200 bg-slate-50/60 text-slate-900 font-bold text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 appearance-none cursor-pointer disabled:opacity-50"
                      >
